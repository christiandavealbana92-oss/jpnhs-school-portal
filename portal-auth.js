...
(function(){
 ...
 const db=firebase.database(),auth=firebase.auth(),school=(new URLSearchParams(location.search).get('school')||'jpnhs').trim();
 ...
 async function signIn(identifier,password,role){
   ...
   const session={uid,schoolID:school,role:link.role,id:link.recordId,name:link.name||email,email:c.user.email};sessionStorage.setItem('schoolPortalAuth',JSON.stringify(session));return session;
 }
 window.PortalAuth={ready:true,db,auth,school,findRecord,createAccount,signIn,logout:async()=>{sessionStorage.removeItem('schoolPortalAuth');localStorage.removeItem('schoolPortalUser');await auth.signOut()}};
})();
