...
const sid=(window.SCHOOL_ID || new URLSearchParams(location.search).get("school") || "").trim();
const path=location.pathname.toLowerCase();
const isMain=path.endsWith("/index.html") || path.endsWith("/") || path==="";
...
const keyFor=()=>{
  ...
  if(role==="student"&&window.ST)return {key:"student:"+String(ST.studentID||""),name:ST.name||"Student",role:"Student"};
  if(role==="teacher"&&window.ME)return {key:"staff:"+String(ME.staffID||ME.id||""),name:ME.name||"Teacher",role:"Teacher"};
  if(role==="registrar"&&window.me)return {key:"staff:"+String(me.staffID||me.id||""),name:me.name||"Registrar",role:"Registrar"};
  return null;
};
...
function buildCompose(){
 if(!state.user)return `<div class="community-compose"><strong>Sign in through your school portal to post.</strong><div class="community-login-note">The main school page is available for viewing. Posting is available from Administrator, Registrar, Teacher, and Student portals after login.</div></div>`;
...
async function notifications(){
 ...
 if(holder)holder.innerHTML=unread.length?unread.map(x=>`<div class="community-notice"><b>New attention notice</b><br>${escC(x.authorName)} called your attention in a post.<br><a href="#communityPage">View the post →</a></div>`).join(""):"";
...
async function injectMain(){
 ...
 ["navRegistrar","navTeacher","navStudent","navAdmin"].forEach(id=>{const el=q("#"+id);if(el)el.href=(id==="navRegistrar"?"registrar.html":id==="navTeacher"?"teacher.html":id==="navStudent"?"student.html":"admin.html")+"?school="+encodeURIComponent(sid)});
